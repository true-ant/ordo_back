...
WITH collapsed AS (
        SELECT DISTINCT ON (op.vendor_id, op.parent_id)
        ofp.id, op.vendor_id, ofp.office_product_category_id
        ...
        ORDER BY op.vendor_id, op.parent_id ASC, ofp."last_order_date" DESC
    ),
    stats AS (
        SELECT
            COUNT(*) as count,
