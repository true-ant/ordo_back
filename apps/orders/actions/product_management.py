...
def unlink_from_parent(p: Product, user: User):
    ...
    ProductParentHistory.objects.create(old_parent=p.parent_id, after=None)
...
def attach_to_parent(p: Product, new_parent: Product, user: User):
    ...
    ProductParentHistory.objects.create(old_parent=old_parent, new_parent=new_parent.pk)
