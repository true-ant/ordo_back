...
class ImplantDirectScraper(Scraper):
    ...
    async def login(self):
        home_resp = await self.getHomePage()
        home_dom = Selector(text=await home_resp.text())
        login_link = home_dom.xpath('//ul/li[@class="authorization-link"]/a/@href').get()
        
        login_resp = await self.getLoginPage(login_link)
        login_dom = Selector(text=await login_resp.text())
        form_key = login_dom.xpath('//form[@id="login-form"]/input[@name="form_key"]/@value').get()
        form_action = login_dom.xpath('//form[@id="login-form"]/@action').get()
        ...
        response = await self.session.post(form_action, headers=headers, data=data)
        print("Log In:", response.status_code)
    ...
    async def clear_cart(self):
        ...
        for product in dom.xpath('//form[@id="form-validate"]//table[@id="shopping-cart-table"]/tbody[@class="cart item"]'):
            ...
            print(f"Product ({_data['data']['id']}) removed from cart:", response.status_code)
    ...
    #     print("ImplantDirect/create_order")
    #     vendor_order_detail = {
    #         "retail_amount": "",
    #         "savings_amount": "",
    #         "subtotal_amount": "",
    ...
    async def confirm_order(self):
        ...
        json_data = {
            ...
            'paymentMethod': {
                ...
                'additional_data': {
                    ...
                    'cc_type': 'VI',
                    'cc_cid': '',
                    'card_id': '413fe324d1adbc400aa4d52e1e72a64118338631',
                },
            },
        }
        ...
        response = await self.session.get('https://store.implantdirect.com/checkout/onepage/success/', headers=headers)
        print("Order Result Response:", response.status_code)

        response_dom = Selector(text=await response.text())
        order_num = response_dom.xpath('//a[@class="order-number"]/strong//text()').get().strip()
        return order_num