...
class BencoScraper(Scraper):
    ...
    def get_ssl_context(cls):
        return ssl.create_default_context(cafile=os.path.join(CERTIFICATE_BASE_PATH, "certificates/benco.pem"))

    async def _get_login_data(self) -> LoginInformation:
        ssl_context = self.get_ssl_context()
        async with self.session.get(
            f"{self.BASE_URL}/Login/Login", headers=PRE_LOGIN_HEADERS, ssl=ssl_context
        ) as resp:
    ...
    async def _check_authenticated(self, resp: ClientResponse) -> bool:
        ...
        await self.session.post(self.BASE_URL, headers=headers, data=data)
    ...
    async def _search_products(
        ...
        async with self.session.post(
            f"{self.BASE_URL}/Search/ChangePage", headers=SEARCH_HEADERS, data=data, ssl=ssl_context
        ) as resp:
            url = str(resp.url)
            response_dom = Selector(text=await resp.text())
            total_size = int(
