...
class PattersonSpider():
    ...
    def parse_products(self, response):
        for product in response.xpath('//table[@aria-label="Search Results Table"]//tr//div[@ng-controller="SearchResultsController"]'):
            product_link = 'https://www.pattersondental.com'+product.xpath(
            ...
            if "ProductFamilyDetails" in product_link:
                ...
                item["name"] = self.textParser(
                    product.xpath('./div[contains(@class, "listViewDescriptionWrapper")]//a[@class="itemTitleDescription"]')
                )
                item["url"] = product_link
                images = product.xpath('./div[contains(@class, "listViewImageWrapper")]/img/@src').extract()
                item["images"] = ";".join(images)
                
                promo_ele = product.xpath('.//span[@class="itemIcons"]/a/span[contains(@data-icontype, "manufacturerPromotionIcon")]')
                if promo_ele:
                    promo_text = self.parse_promo_text(promo_ele)
                    if promo_text: item["Promotion"] = promo_text

                freegood_ele = product.xpath('.//span[@class="itemIcons"]/a/span[contains(@data-icontype, "autoFreeGoodIcon")]')
                if freegood_ele:
                    freegood_text = self.parse_promo_text(freegood_ele)
                    if freegood_text: item["FreeGood"] = freegood_text
    ...
    def parse_family_products(self, family_link):
        ...
        for product in response.xpath('//div[@id="productFamilyDetailsGridBody"]'):
            ...
            item["url"] = 'https://www.pattersondental.com'+product.xpath('.//a[@class="itemTitleDescription"]/@href').get()
            images = product.xpath('.//div[@id="productFamilyDetailsGridBodyColumnOneInnerRowImages"]//img/@src').extract()
            item["images"] = ";".join(images)

            promo_ele = product.xpath('.//span[@class="itemIcons"]/a/span[contains(@class, "manufacturerPromotionIcon")]')
            if promo_ele:
                promo_text = promo_ele.xpath('./@data-title').get()
                if promo_text: item["Promotion"] = promo_text.strip()
            ...
            if freegood_ele:
                ...
                if freegood_text: item["FreeGood"] = freegood_text.strip()
    ...
    def parse_promo_text(self, ele):
        try:
            ...
            response = self.session.get('https://www.pattersondental.com/ItemIcons/GetPromotionIconText', headers=headers, params=params)
